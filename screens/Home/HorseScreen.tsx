...
export const HorseScreen = () => {
  ...
  useEffect(() => {
    if (getToken) {
      axios.get(api.Server + api.getRaceAuth + 4, { headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + getToken() } })
        .then(response => {
          setIsLoading(false);
      ...
      axios.get(api.Server + api.getRace + 4)
        .then(response => {
          setIsLoading(false);
          setData(response.data.pets);
          //console.log(response.data)
  ...
  return (
    <View style={{minHeight: '100%', backgroundColor: colors.background}}>
      <FilterSearch onPressToTop={scrollToTop} title={'Recent Horses'} />
