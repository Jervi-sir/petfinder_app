...
export const RaceLostScreen = ({ route }) => {
  ...
  const fetchPosts = async (page, overwrite = false, wilaya = userWilaya) => {
    //if (loading || !hasMore) return;
    if (loading) return;
    setLoading(true);
    console.log('currentPage= ' + page)

    const url = Api.Server + (BearerToken ? Api.Auth : '') + Api.getLatestLostPets + '?page=' + page + (wilaya ? ('&wilaya_id=' + wilaya) : '');
    console.log(url)
    const headers = {
